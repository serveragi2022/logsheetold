...
Public Class Copyofticketwoutscale
    ...
    Private Sub ExecuteSaveAsDraft(ByVal connectionString As String)
        Using connection As New SqlConnection(connectionString)
            ...
            Try
                ...
                sql = "Update tbllogticket set remarks='" & Trim(txtrems.Text) & "', ticketwhse='" & lblwhse.Text & "', ticketshift='" & lblshift.Text & "', location='" & Trim(cmbloc.Text) & "', columns='" & CInt(numcol.Value) & "', bags='" & txtbags.Text & "', palletnum='" & txtpallet.Text & "', letter1='" & txtletter1.Text & "', astart='" & txtastart.Text & "', letter2='" & txtletter2.Text & "', aend='" & txtaend.Text & "', letter3='" & txtlet1.Text & "', fstart='" & txtfstart.Text & "', letter4='" & txtlet2.Text & "', fend='" & txtfend.Text & "', gseries='" & lblseries.Text & "', cseries='" & lblcancel.Text & "', whsechecker='" & txtchecker.Text & "', forklift='" & cmbfork.Text & "', datemodified=GetDate(), modifiedby='" & login.user & "' where logitemid='" & txtlogitemid.Text & "' and logticketid='" & txtlogticket.Text & "'"
                command.CommandText = sql
                command.ExecuteNonQuery()
                ...
                If tblexistcancel = False Then
                    ...
                    sql = "Create Table " & tbltempcancel & " (logcancelid int NOT NULL PRIMARY KEY IDENTITY(1,1), logsheetid int, logsheetnum nvarchar(MAX), logitemid int, logticketid int, palletnum nvarchar(MAX), letter nvarchar(50), cticketnum int, remarks nvarchar(MAX), grossw float, cticketdate datetime, status int, branch nvarchar(50))"
                    command.CommandText = sql
                    command.ExecuteNonQuery()
                Else
                ...
                For Each row As DataGridViewRow In grdcancel.Rows 'list1 are the list of cancel tickets
                    ...
                    sql = "Insert into " & tbltempcancel & " (logsheetid, logsheetnum, logitemid, logticketid, palletnum, letter, cticketnum, cticketdate, remarks, grossw, status, branch) values ('" & lbllogid.Text & "', '" & lbltemp.Text & txtsearch.Text & "', '" & txtlogitemid.Text & "', '" & txtlogticket.Text & "', '" & txtpallet.Text & "', '" & col4 & "', '" & col0 & "', '" & col1 & "', '" & col2 & "', '" & col3 & "', '1', '" & login.branch & "')"
                    command.CommandText = sql
                    command.ExecuteNonQuery()
                Next
                'cancel///////////////////////////////////////////////////////////////////////
                ...
                If tblexistgood = False Then
                    'create tbltempgood
                    sql = "Create Table " & tbltempgood & " (loggoodid int NOT NULL PRIMARY KEY IDENTITY(1,1), logsheetid int, logsheetnum nvarchar(MAX), logitemid int, logticketid int, palletnum nvarchar(MAX), letter nvarchar(50), gticketnum int, remarks nvarchar(MAX), status int, branch nvarchar(50))"
                    command.CommandText = sql
                    command.ExecuteNonQuery()
                Else
                ...
                For Each item As Object In list8.Items 'list8 are the list of valid tickets
                    ...
                    sql = "Insert into " & tbltempgood & " (logsheetid, logsheetnum, logitemid, logticketid, palletnum, letter, gticketnum, remarks, status, branch) values ('" & lbllogid.Text & "', '" & lbltemp.Text & txtsearch.Text & "', '" & txtlogitemid.Text & "', '" & txtlogticket.Text & "', '" & txtpallet.Text & "', '" & letticket & "', '" & valticket & "', '', '1', '" & login.branch & "')"
                    command.CommandText = sql
                    command.ExecuteNonQuery()
                Next
                'good///////////////////////////////////////////////////////////////////////
                ...
                If tblexistdouble = False Then
                    ...
                    sql = "Create Table " & tbltempdouble & " (logdoubleid int NOT NULL PRIMARY KEY IDENTITY(1,1), logsheetid int, logsheetnum nvarchar(MAX), logitemid int, logticketid int, palletnum nvarchar(MAX), letter nvarchar(50), dticketnum nvarchar(MAX), remarks nvarchar(MAX), dticketdate datetime, status int, branch nvarchar(50))"
                    command.CommandText = sql
                    command.ExecuteNonQuery()
                Else
                ...
                For Each row As DataGridViewRow In grddouble.Rows 'list1 are the list of double tickets
                    ...
                    sql = "Insert into " & tbltempdouble & " (logsheetid, logsheetnum, logitemid, logticketid, palletnum, letter, dticketnum, dticketdate, remarks, status, branch) values ('" & lbllogid.Text & "', '" & lbltemp.Text & txtsearch.Text & "', '" & txtlogitemid.Text & "', '" & txtlogticket.Text & "', '" & txtpallet.Text & "', '" & col3 & "', '" & col0 & "', '" & col1 & "', '" & col2 & "', '1', '" & login.branch & "')"
                    command.CommandText = sql
                    command.ExecuteNonQuery()
                Next
                'double///////////////////////////////////////////////////////////////////////
    ...
    Private Sub btnadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnadd.Click
        Try
            ...
            If cmbreason.Text = "Under Weight" And (Val(txtgross.Text) >= minval Or Val(txtgross.Text) <= 15) And txtgross.Text <> "" Then
                MsgBox("Invalid input of gross weight.", MsgBoxStyle.Exclamation, "")
                txtgross.Focus()
                txtgross.Text = ""
                Exit Sub
            ElseIf cmbreason.Text = "Over Weight" And (Val(txtgross.Text) <= maxval Or Val(txtgross.Text) >= 50) And txtgross.Text <> "" Then
                MsgBox("Invalid input of gross weight.", MsgBoxStyle.Exclamation, "")
                txtgross.Focus()
                txtgross.Text = ""
                Exit Sub
    ...
    Private Sub AddPalletToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AddPalletToolStripMenuItem.Click
        Try
            ...
            If tn.Tag <> "Parent" Then
                ...
                sql = "Select * from tbllogticket where logitemid='" & tn.Tag & "' and tbllogticket.status<>'3'"
                connect()
                cmd = New SqlCommand(sql, conn)
                dr = cmd.ExecuteReader
                While dr.Read
                ...
                If cmbfork.Text = "" Then
                    sql = "Select TOP 2 * from tbllogticket where logsheetnum='" & lbltemp.Text & txtsearch.Text & "' and tbllogticket.status<>'3' and branch='" & login.branch & "' order by logticketid DESC"
                    connect()
                    cmd = New SqlCommand(sql, conn)
                    dr = cmd.ExecuteReader
                    While dr.Read
    ...
    Private Sub ExecuteAddPallet(ByVal connectionString As String)
        Using connection As New SqlConnection(connectionString)
            ...
            Try
                ...
                sql = "Select Count(logticketid) from tbllogticket where logsheetyear='" & lblyear.Text & "' and branch='" & login.branch & "'"
                command.CommandText = sql
                ...
                sql = "Insert into tbllogticket (logsheetyear, logsheetid, logsheetnum, logitemid, ticketdate, palletnum, bags, location, columns, gseries, cseries, qadispo, qarems, cusreserve, customer, ofnum, datecreated, createdby, datemodified, modifiedby, branch, status) values ('" & Val(lblyear.Text) & "', '" & lbllogid.Text & "', '" & lbltemp.Text & Trim(txtsearch.Text) & "', '" & txtlogitemid.Text & "', '" & Format(CDate(lbldate.Text), "yyyy/MM/dd") & "', '" & palreftag & "', '" & bagnum & "', '', '1', '', '', '0', '', '0', '', '', GetDate(), '" & login.user & "', GetDate(), '" & login.user & "', '" & login.branch & "', '1')"
                command.CommandText = sql
                command.ExecuteNonQuery()
    ...
    Public Sub viewticketrange()
        Try
            ...
            sql = "Select * from tbllogrange where logsheetnum='" & lbltemp.Text & txtsearch.Text & "' and status<>'3' order by letter1,arfrom"
            connect()
            cmd = New SqlCommand(sql, conn)
            dr = cmd.ExecuteReader
            While dr.Read
    ...
    Public Sub viewinitialticket()
        Try
            ...
            sql = "Select TOP 1 * from tbllogticket where logsheetnum='" & lbltemp.Text & txtsearch.Text & "' and logticketid<>'" & txtlogticket.Text & "' and tbllogticket.status<>'3' and branch='" & login.branch & "' order by logticketid DESC"
            connect()
            cmd = New SqlCommand(sql, conn)
            dr = cmd.ExecuteReader
            If dr.Read Then
        ...
        Using connection As New SqlConnection(connectionString)
            ...
            Try
                ...
                For Each row As DataGridViewRow In grdcancel.Rows 'list1 are the list of cancel tickets
                    ...
                    sql = "Select * from tbllogcancel where logyear=CAST (YEAR(GetDate()) as nvarchar(50)) and logsheetid='" & lbllogid.Text & "' and logsheetnum='" & lbltemp.Text & txtsearch.Text & "'"
                    sql = sql & " and logitemid='" & txtlogitemid.Text & "' and logticketid='" & txtlogticket.Text & "' and palletnum='" & txtpallet.Text & "'"
                    sql = sql & " and tbllogcancel.branch='" & login.branch & "' and tickettype='" & txttype.Text & "' and letter='" & col4 & "' and cticketnum='" & col0 & "' and cticketdate='" & col1 & "' and remarks='" & col2 & "' and grossw='" & col3 & "' and status='1'"
                    command.CommandText = sql
                    dr = command.ExecuteReader
                    If dr.Read Then
                    ...
                    Else
                        dr.Dispose()
                        sql = "Insert into tbllogcancel (logyear, logsheetid, logsheetnum, logitemid, logticketid, palletnum, tickettype, letter, cticketnum, cticketdate, remarks, grossw, branch, status) "
                        sql = sql & "values (CAST (YEAR(GetDate()) as nvarchar(50)),'" & lbllogid.Text & "','" & lbltemp.Text & txtsearch.Text & "', '" & txtlogitemid.Text & "', '" & txtlogticket.Text & "', '" & txtpallet.Text & "', '" & txttype.Text & "', '" & col4 & "', '" & col0 & "', '" & col1 & "', '" & col2 & "', '" & col3 & "', '" & login.branch & "', '1')"
                        command.CommandText = sql
                        command.ExecuteNonQuery()
                    End If
                    dr.Dispose()
                ...
                For Each item As Object In list8.Items 'list8 are the list of valid tickets
                    ...
                    sql = "Select * from tblloggood where logyear=CAST (YEAR(GetDate()) as nvarchar(50)) and logsheetid='" & lbllogid.Text & "' and logsheetnum='" & lbltemp.Text & txtsearch.Text & "' and logitemid='" & txtlogitemid.Text & "'"
                    sql = sql & " and tblloggood.branch='" & login.branch & "' and logticketid='" & txtlogticket.Text & "' and palletnum='" & txtpallet.Text & "' and tickettype='" & txttype.Text & "' and letter='" & letticket & "' and gticketnum='" & valticket & "' and status='1'"
                    command.CommandText = sql
                    dr = command.ExecuteReader
                    If dr.Read Then
                    ...
                    Else
                        dr.Dispose()
                        sql = "Insert into tblloggood (logyear, logsheetid, logsheetnum, logitemid, logticketid, palletnum, tickettype, letter, gticketnum, remarks, branch, status) "
                        sql = sql & "values (CAST (YEAR(GetDate()) as nvarchar(50)),'" & lbllogid.Text & "','" & lbltemp.Text & txtsearch.Text & "', '" & txtlogitemid.Text & "', '" & txtlogticket.Text & "', '" & txtpallet.Text & "', '" & txttype.Text & "', '" & letticket & "', '" & valticket & "', '', '" & login.branch & "', '1')"
                        command.CommandText = sql
                        command.ExecuteNonQuery()
                    End If
                    dr.Dispose()
                ...
                For Each row As DataGridViewRow In grddouble.Rows 'list6 are the list of double tickets
                    ...
                    sql = "Insert into tbllogdouble (logyear, logsheetid, logsheetnum, logitemid, logticketid, palletnum, tickettype, letter, dticketnum, dticketdate, remarks, branch, status) values (CAST (YEAR(GetDate()) as nvarchar(50)),'" & lbllogid.Text & "','" & lbltemp.Text & txtsearch.Text & "', '" & txtlogitemid.Text & "', '" & txtlogticket.Text & "', '" & txtpallet.Text & "', '" & txttype.Text & "', '" & col3 & "', '" & col0 & "', '" & col1 & "', '" & col2 & "', '" & login.branch & "', '1')"
                    command.CommandText = sql
                    command.ExecuteNonQuery()
                Next

                'update add to location datetime
                sql = "Update tbllogticket set remarks='" & Trim(txtrems.Text) & "', addtoloc=GetDate(), ticketwhse='" & lblwhse.Text & "', ticketshift='" & lblshift.Text & "', location='" & Trim(cmbloc.Text) & "', columns='" & CInt(numcol.Value) & "', bags='" & txtbags.Text & "', palletnum='" & txtpallet.Text & "', letter1='" & txtletter1.Text & "', astart='" & txtastart.Text & "', letter2='" & txtletter2.Text & "', aend='" & txtaend.Text & "', letter3='" & txtlet1.Text & "', fstart='" & txtfstart.Text & "', letter4='" & txtlet2.Text & "', fend='" & txtfend.Text & "', gseries='" & lblseries.Text & "', cseries='" & lblcancel.Text & "', whsechecker='" & txtchecker.Text & "', forklift='" & cmbfork.Text & "', datemodified=GetDate(), modifiedby='" & login.user & "' where logitemid='" & txtlogitemid.Text & "' and logticketid='" & txtlogticket.Text & "'"
                command.CommandText = sql
                command.ExecuteNonQuery()
                ...
                sql = "Update tbllogrange set frto='" & Val(txtfend.Text) & "', ftotal='" & ftot & "', status='1' where logsheetnum='" & lbltemp.Text & txtsearch.Text & "' and status<>'3'"
                command.CommandText = sql
                command.ExecuteNonQuery()
                '////////////////////////////////////////////////////////////////////////////////
    ...
    Private Sub btnbin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnprintbin.Click
        Try
            ...
            If lblbin.Text <> "" Then
                ...
                sql = "Select * from tbllogticket where logitemid='" & lblbin.Text & "' and addtoloc is NULL and tbllogticket.status<>'3'"
                connect()
                cmd = New SqlCommand(sql, conn)
                dr = cmd.ExecuteReader
                If dr.Read Then
    ...
    Private Sub cmbloc_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbloc.Leave
        If Trim(cmbloc.Text) <> "" Then
            ...
            sql = "Select TOP 4 * from tbllogticket where status='1' and location='" & Trim(cmbloc.Text) & "' and branch='" & login.branch & "' order by logticketid DESC"
            connect()
            cmd = New SqlCommand(sql, conn)
            dr = cmd.ExecuteReader
            While dr.Read
    ...
    Private Sub btnlogconfirm_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnlogconfirm.Click
        Try
            ...
            sql = sql & " where tbllogsheet.branch='" & login.branch & "' and tbllogsheet.logsheetnum='" & lbltemp.Text & txtsearch.Text & "' and (tbllogticket.addtoloc is NULL or tbllogticket.qadispo='0' or tbllogticket.qadispo='3') and tbllogticket.status<>'3'"
            connect()
            cmd = New SqlCommand(sql, conn)
            dr = cmd.ExecuteReader
            If dr.Read Then
    ...
    Private Sub btnprintlog_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnprintlog.Click
        Try
            sql = "Select * from tbllogticket where logsheetnum='" & lbltemp.Text & txtsearch.Text & "' and (qadispo='0' or qadispo='3') and tbllogticket.status<>'3' and branch='" & login.branch & "'"
            connect()
            cmd = New SqlCommand(sql, conn)
            dr = cmd.ExecuteReader
            If dr.Read Then
    ...
    Private Sub btncomp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btncomp.Click
        Try
            If btncomp.Text = "Complete" Then
                ...
                sql = "Select * from tbllogticket where logsheetnum='" & lbltemp.Text & txtsearch.Text & "' and tbllogticket.status<>'3' and branch='" & login.branch & "'"
                connect()
                cmd = New SqlCommand(sql, conn)
                dr = cmd.ExecuteReader
                While dr.Read
    ...
    Public Function validateastart()
        Try
            ...
            If txttype.Text = "Inkjet" Then
                sql = "Select tbllogsheet.logsheetyear, tbllogsheet.whsename, tbllogsheet.palletizer, tblloggood.logticketid, tblloggood.tickettype, tblloggood.letter, tblloggood.gticketnum, tblloggood.status"
                sql = sql & " from tbllogsheet inner join tblloggood on tbllogsheet.logsheetid=tblloggood.logsheetid"
                sql = sql & " where tbllogsheet.branch='" & login.branch & "' and tbllogsheet.logsheetyear=Year(Getdate()) and tbllogsheet.whsename='" & lblwhse.Text & "' and tbllogsheet.palletizer='" & lblline.Text & "'"
                sql = sql & " and (tblloggood.branch='" & login.branch & "' and tblloggood.tickettype='" & txttype.Text & "' and tblloggood.letter='" & txtletter1.Text & "' and tblloggood.gticketnum='" & Trim(txtastart.Text) & "' and tblloggood.status<>'3')"

                sql = sql & " UNION "

                sql = sql & "Select tbllogsheet.logsheetyear, tbllogsheet.whsename, tbllogsheet.palletizer, tbllogcancel.logticketid, tbllogcancel.tickettype, tbllogcancel.letter, tbllogcancel.cticketnum, tbllogcancel.status"
                sql = sql & " from tbllogsheet inner join tbllogcancel on tbllogsheet.logsheetid = tbllogcancel.logsheetid"
                sql = sql & " where tbllogsheet.branch='" & login.branch & "' and tbllogsheet.logsheetyear=Year(Getdate()) and tbllogsheet.whsename='" & lblwhse.Text & "' and tbllogsheet.palletizer='" & lblline.Text & "'"
                sql = sql & " and (tbllogcancel.branch='" & login.branch & "' and tbllogcancel.tickettype='" & txttype.Text & "' and tbllogcancel.letter='" & txtletter1.Text & "' and tbllogcancel.cticketnum='" & Trim(txtastart.Text) & "' and tbllogcancel.status<>'3')"

                sql = sql & " UNION "

                sql = sql & "Select tbllogsheet.logsheetyear, tbllogsheet.whsename, tbllogsheet.palletizer, tbllogdouble.logticketid, tbllogdouble.tickettype, tbllogdouble.letter, tbllogdouble.dticketnum, tbllogdouble.status"
                sql = sql & " from tbllogsheet inner join tbllogdouble on tbllogsheet.logsheetid = tbllogdouble.logsheetid"
                sql = sql & " where tbllogsheet.branch='" & login.branch & "' and tbllogsheet.logsheetyear=Year(Getdate()) and tbllogsheet.whsename='" & lblwhse.Text & "' and tbllogsheet.palletizer='" & lblline.Text & "'"
                sql = sql & " and (tbllogdouble.branch='" & login.branch & "' and tbllogdouble.tickettype='" & txttype.Text & "' and tbllogdouble.letter='" & txtletter1.Text & "' and tbllogdouble.dticketnum='" & Trim(txtastart.Text) & "' and tbllogdouble.status<>'3')"
            ...
            Else
                ...
                sql = "Select tblloggood.logticketid from tblloggood where (tblloggood.branch='" & login.branch & "' and tblloggood.tickettype='" & txttype.Text & "' and tblloggood.letter='" & txtletter1.Text & "' and tblloggood.gticketnum='" & Trim(txtastart.Text) & "' and tblloggood.status<>'3')"

                sql = sql & " union "

                sql = "Select tbllogcancel.logticketid from tbllogcancel where (tbllogcancel.branch='" & login.branch & "' and tbllogcancel.tickettype='" & txttype.Text & "' and tbllogcancel.letter='" & txtletter1.Text & "' and tbllogcancel.cticketnum='" & Trim(txtastart.Text) & "' and tbllogcancel.status<>'3')"

                sql = sql & " union "

                sql = "Select tbllogdouble.logticketid from tbllogdouble where (tbllogdouble.branch='" & login.branch & "' and tbllogdouble.tickettype='" & txttype.Text & "' and tbllogdouble.letter='" & txtletter1.Text & "' and tbllogdouble.dticketnum='" & Trim(txtastart.Text) & "' and tbllogdouble.status<>'3')"
            End If
    ...
    Private Sub PrintPalletTagToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PrintPalletTagToolStripMenuItem.Click
        ...
        If tn.Tag <> "Location" And tn.Tag <> "Column" Then
            ...
            sql = "Select * from tbllogticket where palletnum='" & tn.Text.Substring(2, tn.Text.Length - 2) & "' and branch='" & login.branch & "' and tbllogticket.status<>'3'"
            connect()
            cmd = New SqlCommand(sql, conn)
            dr = cmd.ExecuteReader
            If dr.Read Then
