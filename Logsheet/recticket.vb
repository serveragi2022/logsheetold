...
Public Class recticket
    ...
    Public tickettransfercnf As Boolean, ticketline As String, trems As String
    ...
    Private Sub ExecuteViewLine(ByVal connectionString As String)
        Using connection As New SqlConnection(connectionString)
            ...
            Try
                ...
                sql = "Select * from tblwhse where whsename='" & lblwhse.Text & "' and branch='" & login.branch & "'"
                ...
                sql = "Select s.palletizer,i.itemname,i.logitemid,i.status from tbllogitem i right outer join tbllogsheet s on s.logsheetid=i.logsheetid"
                sql = sql & " where s.logsheetid='" & lbllogid.Text & "' and i.status<>'3'"
                command.CommandText = sql
                dr = command.ExecuteReader
                While dr.Read
    ...
    Public Sub errortext()
        Try
            ...
            For Each rowcnl As DataGridViewRow In grdcancel.Rows
                ...
                For Each itemcnl As Object In list7.Items
    ...
    Private Sub ExecuteSaveAsDraft(ByVal connectionString As String)
        Using connection As New SqlConnection(connectionString)
            ...
            Try
                ...
                sql = sql & " bags='" & txtbags.Text & "', palletnum='" & txtpallet.Text & "', letter1='" & txtletter1.Text & "',"
                sql = sql & " astart='" & txtastart.Text & "', letter2='" & txtletter2.Text & "', aend='" & txtaend.Text & "',"
                sql = sql & " letter3='" & txtlet1.Text & "', fstart='" & txtfstart.Text & "', letter4='" & txtlet2.Text & "',"
                sql = sql & " fend='" & txtfend.Text & "', gseries='" & lblseries.Text & "', cseries='" & lblcancel.Text & "',"
                sql = sql & " whsechecker='" & txtchecker.Text & "', forklift='" & cmbfork.Text & "', datemodified=GetDate(),"
                sql = sql & " modifiedby='" & login.user & "' where logitemid='" & txtlogitemid.Text & "' and logticketid='" & txtlogticket.Text & "'"
                command.CommandText = sql
                ...
                For Each row As DataGridViewRow In grddouble.Rows 'list1 are the list of double tickets
                    ...
                    sql = sql & " values ('" & lbllogid.Text & "', '" & txtlogitemid.Text & "', '" & txtlogticket.Text & "', '" & txtpallet.Text & "', '" & col3 & "',"
                    sql = sql & " '" & col0 & "', '" & Format(grddouble.Rows(row.Index).Cells(1).Value, "yyyy/MM/dd HH:mm") & "', '" & col2 & "', '1')"
                    command.CommandText = sql
                    command.ExecuteNonQuery()
                Next
                'double///////////////////////////////////////////////////////////////////////
    ...
    Private Sub btnadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnadd.Click
        Try
            If Trim(txtcancel.Text) = "" Then
                ...
                txtcancel.Focus()
                Exit Sub
            ElseIf Trim(txtletcancel.Text) = "" Then
                Me.Cursor = Cursors.Default
                MsgBox("Input letter first.", MsgBoxStyle.Exclamation, "")
                txtletcancel.Focus()
                Exit Sub
            ...
            'check if nasa list ng list7 kasi dito cover nya lahat ng range astart to fend
            ...
            If nasalist = False Then
                'wala sa list8 ng goods
                MsgBox("Invalid ticket. Ticket is not within the range.", MsgBoxStyle.Exclamation, "")
                txtcancel.Text = ""
                txtcancel.Focus()
                Exit Sub
            End If
            ...
            If Trim(cmbreason.Text).ToString.Contains("Double") = True Then
                ...
                Else
                    ...
                    sql = sql & " where s.logsheetyear='" & Val(lblyear.Text) & "' and g.tickettype='Receive' and g.logsheetid='" & lbllogid.Text & "'"
                    sql = sql & " and s.branch='" & login.branch & "' and g.letter='" & txtletcancel.Text & "' and g.gticketnum='" & txtcancel.Text & "' and g.status<>'3'"
                    connect()
                    cmd = New SqlCommand(sql, conn)
                    ...
                    sql = sql & " where s.logsheetyear='" & Val(lblyear.Text) & "' and c.tickettype='Receive' and c.logsheetid='" & lbllogid.Text & "'"
                    sql = sql & " and s.branch='" & login.branch & "' and c.letter='" & txtletcancel.Text & "' and c.cticketnum='" & txtcancel.Text & "' and c.status<>'3'"
                    connect()
                    cmd = New SqlCommand(sql, conn)
                    ...
                    If checkgood = 0 And checkcancel = 0 Then
            ...
            Else
                ...
                Dim containsgood As Boolean = False
                For Each item As Object In list8.Items
                    If Val(item) = Val(txtcancel.Text) Then
                        containsgood = True
                        Exit For
                ...
                If containsgood = True Then
                    Me.Cursor = Cursors.Default
                    MsgBox("Invalid input. Ticket is already in good tickets.", MsgBoxStyle.Exclamation, "")
                    txtcancel.Text = ""
                    txtcancel.Focus()
                    Exit Sub
                End If
                ...
                If containslist = True Then
                    Me.Cursor = Cursors.Default
                    MsgBox("Invalid input. Ticket is already cancelled.", MsgBoxStyle.Exclamation, "")
                    txtcancel.Text = ""
                    txtcancel.Focus()
                Else
    ...
    Private Sub AddPalletToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AddPalletToolStripMenuItem.Click
        Try
            ...
            If tn.Tag <> "Parent" Then
                ...
                sql = "Select addtoloc, logticketid from tbllogticket where logitemid='" & tn.Tag & "' and tbllogticket.status<>'3'"
                connect()
                cmd = New SqlCommand(sql, conn)
                dr = cmd.ExecuteReader
                While dr.Read
                ...
                remain = Val(txtprod.Text) - rec_bagIN(lbltemp.Text & Trim(txtsearch.Text))
                ...
                If checkpending = False Then
                    If remain > 0 Then
                        ...
                        confirmsave.GroupBox1.Text = login.wgroup
                        confirmsave.ShowDialog()
                        If tickettransfercnf = False Then
                            MsgBox("User cancelled adding pallet #.", MsgBoxStyle.Information, "")
                            defaultform()
                            Exit Sub
                        End If

                        If remain >= Val(lblwhse.Tag) Then
                            txtbags.Text = Val(lblwhse.Tag)
                        Else
                            txtbags.Text = remain
                        End If
                    ...
                    Else
                        MsgBox("Cannot add pallet. " & Val(txtprod.Text) & " bags is already added.", MsgBoxStyle.Exclamation, "")
                        ViewLocationToolStripMenuItem.PerformClick()
                        Exit Sub
                    End If
                ...
                viewticketrange()
                'viewinitialticket() hindi pwede ito kasi pwedeng iba yung ilagay nila sa grd

                'txtletgood.Text = txtletter1.Text
                'txtletcancel.Text = txtletter1.Text
                ...
                If txttype.Text <> "Inkjet" And txttype.Text <> "Receive" Then
                ...
                If cmbfork.Text = "" Then
                    sql = "Select TOP 2 t.forklift from tbllogticket t right outer join tbllogsheet s on t.logsheetid=s.logsheetid"
                    sql = sql & " where t.logsheetid='" & lbllogid.Text & "' and t.status<>'3' and s.branch='" & login.branch & "' order by t.logticketid DESC"
                    connect()
                    cmd = New SqlCommand(sql, conn)
                    dr = cmd.ExecuteReader
                    While dr.Read
            ...
            For Each row As DataGridViewRow In grdgood.Rows
                list8.Items.Add(grdgood.Rows(row.Index).Cells(0).Value & " " & grdgood.Rows(row.Index).Cells(1).Value)
            Next
            genticket()
    ...
    Private Sub ExecuteAddPallet(ByVal connectionString As String)
        Using connection As New SqlConnection(connectionString)
            ...
            Try
                ...
                sql = "Select Count(t.logticketid) from tbllogticket t right outer join tbllogsheet s on s.logsheetid=t.logsheetid"
                ...
                sql = "Insert into tbllogticket (logsheetid, logitemid, palletnum, bags, location, columns,"
                sql = sql & " gseries, cseries, qadispo, qarems, cusreserve, customer, ofnum, datecreated, createdby, datemodified, modifiedby, status) "
                sql = sql & " values ('" & lbllogid.Text & "', '" & txtlogitemid.Text & "', '" & palreftag & "', '" & Val(txtbags.Text) & "', '', '1', '', '', '0', '', "
                sql = sql & " '0', '', '', GetDate(), '" & login.user & "', GetDate(), '" & login.user & "', '1')"
                command.CommandText = sql
                command.ExecuteNonQuery()
            ...
            Try
                ...
                If checkpending = True Then
                    ...
                    If tblexistcancel = True Then
                        sql = "Select cticketnum, letter, cticketdate, remarks, grossw from " & tbltempnamecancel & " where logticketid='" & txtlogticket.Text & "'"
                        command.CommandText = sql
                        dr = command.ExecuteReader
                        While dr.Read
                            list1.Items.Add(dr("cticketnum") & " " & dr("letter"))
                            '//list11.Items.Add(dr("cticketnum") & " " & dr("letter"))
                    ...
                    If tblexistgood = True Then
                        sql = "Select gticketnum, letter from " & tbltempnamegood & " where logticketid='" & txtlogticket.Text & "'"
                        command.CommandText = sql
                        dr = command.ExecuteReader
                        While dr.Read
                            list8.Items.Add(dr("gticketnum") & " " & dr("letter"))
                            '//list11.Items.Add(dr("gticketnum") & " " & dr("letter"))
                            grdgood.Rows.Add(Val(dr("gticketnum")), dr("letter"))
                        End While
                        dr.Dispose()

                        grdgood.SuspendLayout()
                        grdgood.ResumeLayout()
                    End If    'good////////////////////////////////////////////////////////////////////////////////
                    ...
                    If tblexistdouble = True Then
                        sql = "Select dticketnum, letter, dticketdate, remarks from " & tbltempnamedouble & " where logticketid='" & txtlogticket.Text & "'"
                        command.CommandText = sql
                        dr = command.ExecuteReader
                        While dr.Read
                            list6.Items.Add(dr("dticketnum") & " " & dr("letter"))
                            '//list11.Items.Add(dr("dticketnum") & " " & dr("letter"))
                ...
                '// list all ticket sa logsheet good cancel or double
                sql = "Select gticketnum, letter from tblloggood where logsheetid='" & lbllogid.Text & "' and status<>'3'"
                command.CommandText = sql
                dr = command.ExecuteReader
                While dr.Read
                    list11.Items.Add(dr("gticketnum") & " " & dr("letter"))
                ...
                sql = "Select cticketnum, letter from tbllogcancel where logsheetid='" & lbllogid.Text & "' and status<>'3'"
                command.CommandText = sql
                dr = command.ExecuteReader
                While dr.Read
                    list11.Items.Add(dr("cticketnum") & " " & dr("letter"))
    ...
    Public Sub viewinitialticket()
        Try
            ...
            sql = "Select TOP 1 t.fend,t.letter4 from tbllogticket t right outer join tbllogsheet s on s.logsheetid=t.logsheetid"
            sql = sql & " where t.logsheetid='" & lbllogid.Text & "' and t.logticketid<>'" & txtlogticket.Text & "'"
            sql = sql & " and t.status<>'3' and s.branch='" & login.branch & "' order by t.logticketid DESC"
            connect()
            cmd = New SqlCommand(sql, conn)
            dr = cmd.ExecuteReader
            If dr.Read Then
    ...
    Private Sub btnloc_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnloc.Click
        Try
            ...
            'check if yung txtbag = grdgood.rows.count
            If Val(txtbags.Text) > grdgood.Rows.Count Then
                MsgBox("No. of good tickets (" & grdgood.Rows.Count & ") is less than the no. of bags per pallet (" & Val(txtbags.Text) & ").", MsgBoxStyle.Exclamation, "")
                Exit Sub
            ElseIf Val(txtbags.Text) < grdgood.Rows.Count Then
                MsgBox("No. of good tickets (" & grdgood.Rows.Count & ") is greater than the no. of bags per pallet (" & Val(txtbags.Text) & ").", MsgBoxStyle.Exclamation, "")
                Exit Sub
            End If

            'check if kumpleto details
            If Trim(cmbloc.Text) <> "" And Trim(cmbfork.Text) <> "" And Trim(lblseries.Text) <> "" Then
        ...
        Using connection As New SqlConnection(connectionString)
            ...
            Try
                ...
                For Each row As DataGridViewRow In grdcancel.Rows 'list1 are the list of cancel tickets
                    ...
                    sql = "Select c.cticketnum from tbllogcancel c right outer join tbllogsheet s on s.logsheetid=c.logsheetid"
                    sql = sql & " where s.logsheetyear=CAST(YEAR(GetDate()) as nvarchar(50)) and c.logsheetid='" & lbllogid.Text & "' and c.logitemid='" & txtlogitemid.Text & "' "
                    sql = sql & " and c.logticketid='" & txtlogticket.Text & "' and s.branch='" & login.branch & "' and c.tickettype='" & txttype.Text & "' and c.letter='" & col4 & "' and c.cticketnum='" & col0 & "' "
                ...
                For Each item As Object In list8.Items 'list8 are the list of valid tickets
                    ...
                    sql = "Select g.gticketnum from tblloggood g right outer join tbllogsheet s on s.logsheetid=g.logsheetid"
                    sql = sql & " where s.logsheetyear=CAST(YEAR(GetDate()) as nvarchar(50)) and g.logsheetid='" & lbllogid.Text & "'"
                    sql = sql & " and g.logitemid='" & txtlogitemid.Text & "' and s.branch='" & login.branch & "' and g.logticketid='" & txtlogticket.Text & "'"
                    ...
                    Else
                        ...
                        sql = sql & " values ('" & lbllogid.Text & "', '" & txtlogitemid.Text & "', '" & txtlogticket.Text & "', '" & txtpallet.Text & "', '" & txttype.Text & "', '" & letticket & "', '" & valticket & "', '', '1')"
                        command.CommandText = sql
                        command.ExecuteNonQuery()
                    End If
                    dr.Dispose()
                ...
                sql = "Update tbllogticket set remarks='" & Trim(txtrems.Text) & "', addtoloc=GetDate(), location='" & Trim(cmbloc.Text) & "', columns='" & CInt(numcol.Value) & "', bags='" & txtbags.Text & "', palletnum='" & txtpallet.Text & "',"
                sql = sql & " letter1='" & grdgood.Rows(0).Cells(1).Value & "', astart='" & grdgood.Rows(0).Cells(0).Value & "', letter2='" & grdgood.Rows(0).Cells(1).Value & "',"
                sql = sql & " aend='" & grdgood.Rows(0).Cells(0).Value & "', letter3='" & grdgood.Rows(grdgood.RowCount - 1).Cells(1).Value & "', fstart='" & grdgood.Rows(grdgood.RowCount - 1).Cells(0).Value & "',"
                sql = sql & " letter4='" & grdgood.Rows(grdgood.RowCount - 1).Cells(1).Value & "', fend='" & grdgood.Rows(grdgood.RowCount - 1).Cells(0).Value & "',"
                ...
                sql = sql & " binnum=(Select binnum from tbllogsheet where logsheetnum='" & lbltemp.Text & txtsearch.Text & "' and status='1')"
    ...
    Private Sub cmbloc_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbloc.Leave
        If Trim(cmbloc.Text) <> "" Then
            ...
            'sql = "Select * from tbllogticket right outer join tbllogitem on tbllogticket.logitemid=tbllogitem.logitemid where tbllogticket.status='1' and tbllogticket.location='" & Trim(cmbloc.Text) & "' and tbllogitem.itemname<>'" & txtitem.Text & "' and tbllogticket.status<>'3'"
            'connect()
            'cmd = New SqlCommand(sql, conn)
            'dr = cmd.ExecuteReader
            'If dr.Read Then
    ...
    Private Sub btnprintlog_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnprintlog.Click
        Try
            ...
            sql = sql & " where t.logsheetid='" & lbllogid.Text & "' and (t.qadispo='0' or t.qadispo='3') and t.status<>'3' and s.branch='" & login.branch & "'"
            connect()
            cmd = New SqlCommand(sql, conn)
            dr = cmd.ExecuteReader
            If dr.Read Then
    ...
    Private Sub btncomp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btncomp.Click
        Try
            If btncomp.Text = "Complete" Then
                ...
                sql = "Select t.addtoloc,t.palletnum from tbllogticket t right outer join tbllogsheet s on s.logsheetid=t.logsheetid"
                sql = sql & " where t.logsheetid='" & lbllogid.Text & "' and t.status<>'3' and s.branch='" & login.branch & "'"
                connect()
                cmd = New SqlCommand(sql, conn)
                dr = cmd.ExecuteReader
                While dr.Read
                ...
                If TreeView2.Nodes(0).Nodes.Count <> 0 Then
                    'check kung ung count is same sa mga nasave na tickets
                    Dim logqty As Integer = 0, ticketqty As Integer = 0
                    sql = "select qty from tbllogsheet where logsheetid='" & lbllogid.Text & "'"
                    connect()
                    cmd = New SqlCommand(sql, conn)
                    dr = cmd.ExecuteReader
                    If dr.Read Then
                        logqty = Val(dr("qty").ToString)
                    End If
                    dr.Dispose()
                    cmd.Dispose()
                    conn.Close()
                    ...
                    sql = sql & " (Select loggoodid from tblloggood where logsheetid='" & lbllogid.Text & "' and status<>'3' union Select logcancelid from tbllogcancel where logsheetid='" & lbllogid.Text & "' and status<>'3')  s"
                    connect()
                    cmd = New SqlCommand(sql, conn)
                    ticketqty = cmd.ExecuteScalar
                    cmd.Dispose()
                    conn.Close()

                    If logqty <> ticketqty Then
                        MsgBox("Incomplete. " & ticketqty & " bags only out of " & logqty & " bags.", MsgBoxStyle.Exclamation, "")
                        Exit Sub
                    End If
    ...
    Private Sub btnselect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnselect.Click
        ...
        End If
        txtsearch.ReadOnly = False
        recselect.winfrm = Me.Name
        recselect.ShowDialog()
        txtsearch.ReadOnly = False
    ...
    Private Sub ViewPalletToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ViewPalletToolStripMenuItem.Click
        Try
            ...
            If tn.Tag <> "Location" And tn.Tag <> "Column" Then
                ...
                viewpallet.Label4.Visible = False
                viewpallet.txtletter1.Visible = False
                viewpallet.txtastart.Visible = False
                viewpallet.Label5.Visible = False
                viewpallet.txtletter2.Visible = False
                viewpallet.txtaend.Visible = False
                viewpallet.Label15.Visible = False
                viewpallet.txtlet1.Visible = False
                viewpallet.txtfstart.Visible = False
                viewpallet.Label14.Visible = False
                viewpallet.txtlet2.Visible = False
                viewpallet.txtfend.Visible = False
        ...
        Using connection As New SqlConnection(connectionString)
            ...
            Try
                ...
                If dr.Read Then
                    ...
                    viewpallet.txtbags.Text = dr("bags").ToString
                ...
                If viewpallet.txttype.Text <> "Inkjet" And viewpallet.txttype.Text <> "Receive" Then
                    'view flour type
    ...
    Public Function ifkulangticket()
        ...
        If txtrange.Text.ToString.Contains(";") = False Then
            If (Val(lblrto.Text) - Val(txtfstart.Text)) + 1 < Val(txtbags.Text) Then
                MsgBox("Tickets left in ticket range are not enough. Must add another ticket range.", MsgBoxStyle.Exclamation, "")
                viewinitialticket()
                txtastart.Focus()
                kulang = True
            Else
    ...
    Public Function validateastart()
        Try
            ...
            Else
                '/sql = "Select tblloggood.logticketid from tblloggood full outer join tbllogcancel on tblloggood.logticketid = tbllogcancel.logticketid full outer join tbllogdouble on tblloggood.logticketid = tbllogdouble.logticketid"
                '/sql = sql & " where ((tblloggood.branch='" & login.branch & "' and tblloggood.tickettype='" & txttype.Text & "' and tblloggood.letter='" & txtletter1.Text & "' and tblloggood.gticketnum='" & Trim(txtastart.Text) & "' and tblloggood.status<>'3')"
                '/sql = sql & " or (tbllogcancel.branch='" & login.branch & "' and tbllogcancel.tickettype='" & txttype.Text & "' and tbllogcancel.letter='" & txtletter1.Text & "' and tbllogcancel.cticketnum='" & Trim(txtastart.Text) & "' and tbllogcancel.status<>'3'))"
    ...
    Private Sub btnpick_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnpick.Click
        recticketpick.txtpallet.Text = txtpallet.Text
        recticketpick.txtlet.Text = txtletgood.Text
        recticketpick.ShowDialog()
    End Sub

    Private Sub btnrevgood_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnrevgood.Click
        For Each row As DataGridViewRow In grdgood.SelectedRows
            Dim listindex As Integer = row.Index
            Dim listitem As Integer = Val(grdgood.Rows(row.Index).Cells(0).Value)

            grdgood.Rows.Remove(row)
            '/grdgood.SortedColumn

            For l_index As Integer = 0 To list8.Items.Count - 1
                Dim l_text As String = CStr(list8.Items(l_index))
                If listitem = Val(l_text) Then
                    list8.Items.RemoveAt(l_index)
                    Exit For
                End If
            Next
        Next
    ...
    Private Sub btngood_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btngood.Click
        Try
            If Trim(txtgood.Text) = "" Then
                Me.Cursor = Cursors.Default
                MsgBox("Input ticket number first.", MsgBoxStyle.Exclamation, "")
                txtgood.Focus()
                Exit Sub

            ElseIf Trim(txtletgood.Text) = "" Then
                Me.Cursor = Cursors.Default
                MsgBox("Input letter first.", MsgBoxStyle.Exclamation, "")
                txtletgood.Focus()
                Exit Sub
            ...
            If Val(txtgood.Text) <> 0 Then 'And Val(txtastart.Text) <> 0 Then
                'check if yung txtbag = grdgood.rows.count
                If Val(txtbags.Text) = grdgood.Rows.Count Then
                    MsgBox("No. of bags is already " & Val(txtbags.Text) & ".", MsgBoxStyle.Exclamation, "")
                    Exit Sub
                End If
                ...
                Dim nasalist As Boolean = False, itemwlet As String = "", lettr As String = ""
                For Each item As Object In list7.Items
                    If Val(item) = Val(txtgood.Text) Then
                        nasalist = True
                        itemwlet = item
                        lettr = item
                        lettr = lettr.Substring(lettr.Length - 1)
                ...
                Dim winthingrdgood As Boolean = False
                For Each item As Object In list8.Items
                    If item = Val(txtgood.Text) & " " & txtletgood.Text Then
                        winthingrdgood = True
                        Exit For
                    End If
                Next

                If winthingrdgood = True Then
                    MsgBox("Ticket is already added.", MsgBoxStyle.Exclamation, "")
                    txtgood.Focus()
                    Exit Sub
                End If
                ...
                Else
                    ...
                    sql = "Select Count(g.loggoodid) from tblloggood g right outer join tbllogsheet s on s.logsheetid=g.logsheetid"
                    sql = sql & " where s.logsheetyear='" & Val(lblyear.Text) & "' and g.tickettype='Receive' and g.logsheetid='" & lbllogid.Text & "'"
                    sql = sql & " and s.branch='" & login.branch & "' and g.letter='" & txtletgood.Text & "' and g.gticketnum='" & txtgood.Text & "' and g.status<>'3'"
                    connect()
                    cmd = New SqlCommand(sql, conn)
                    checkgood = cmd.ExecuteScalar
                    cmd.Dispose()
                    ...
                    sql = "select Count(c.logcancelid) from tbllogcancel c right outer join tbllogsheet s on s.logsheetid=c.logsheetid"
                    sql = sql & " where s.logsheetyear='" & Val(lblyear.Text) & "' and c.tickettype='Receive' and c.logsheetid='" & lbllogid.Text & "'"
                    sql = sql & " and s.branch='" & login.branch & "' and c.letter='" & txtletgood.Text & "' and c.cticketnum='" & txtgood.Text & "' and c.status<>'3'"
                    connect()
                    cmd = New SqlCommand(sql, conn)
                    checkcancel = cmd.ExecuteScalar
                    cmd.Dispose()
                    ...
                    If checkgood = 0 And checkcancel = 0 Then
                        grdgood.Rows.Add(Val(txtgood.Text), lettr, 0)
                        grdgood.Sort(grdgood.Columns(0), System.ComponentModel.ListSortDirection.Ascending)
                        txtgood.Text = ""

                        list8.Items.Clear()
                        For Each row As DataGridViewRow In grdgood.Rows
                            list8.Items.Add(grdgood.Rows(row.Index).Cells(0).Value & " " & lettr) 'itemwlet
                        Next
                    Else
                        MsgBox("Ticket is already exist.", MsgBoxStyle.Exclamation, "")
                        txtgood.Text = ""
    ...
    Private Sub txtgood_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtgood.KeyPress
        If Asc(e.KeyChar) = 39 Then
            e.Handled = True
        ElseIf Asc(e.KeyChar) = Windows.Forms.Keys.Enter Then
    ...
    Private Sub txtgood_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtgood.TextChanged

    End Sub

    Private Sub txtletgood_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtletgood.KeyPress
        If Asc(e.KeyChar) = 39 Then
            e.Handled = True
        End If
    End Sub
